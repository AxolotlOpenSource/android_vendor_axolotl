...
func init() {
	android.RegisterModuleType("axolotl_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = axolotlExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = axolotlExpandVariables(ctx, dep_file)
	...
	cmd := axolotlExpandVariables(ctx, String(g.properties.Cmd))
